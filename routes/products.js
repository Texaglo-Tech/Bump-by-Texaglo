...
/* POST Product get collectibles */
router.post('/get_collectibles', auth.isAuthenticated, (req, res) => {
...
router.post('/get_product', (req, res) => {
	const { product_id } = req.body;	// type: web or app
	Product.findOne({
		product_id
	}, async (error, product) => {
		if (error) return res.status(500).json({
...
router.post("/ai_chat", async (req, res) => {
	...
	if(type == "chatpdf"){
		...
		let chatpdf_source = '';
		if(product_id == "technoking") chatpdf_source = "source id"
		else{
