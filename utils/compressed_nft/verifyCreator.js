...
const { MetadataArgs, Creator } = require("@metaplex-foundation/mpl-bubblegum");
const {
  AccountMeta,
  PublicKey,
  TransactionMessage,
...
const verifyCreator = async () => {
  ...
  const collectionMetadata = keys.collectionMetadataAccount;
  const collectionMasterEdition = keys.collectionMasterEditionAccount;

  const assetIdTestAddress = keys.assetIdTestAddress;
  const assetIdUserAddress = keys.assetIdUserAddress;
   ...
   */
  const creators = asset.creators.map(creator => {
    return {
  ...
  const slicedProof = assetProof.proof
    .map((node) => ({
